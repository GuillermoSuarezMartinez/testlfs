...
namespace Orbita.Compresion
{
    ...
    public class OrbitaZip : IDisposable
    {
        ...
        {
            ExtractZip(zipFileName, targetDirectory, Sobreescritura.Siempre, null, fileFilter, null, restoreDateTimeOnExtract_);
        }
        ...
        public void ExtractZip(string zipFileName, string targetDirectory,
                               Sobreescritura overwrite, ConfirmOverwriteDelegate confirmDelegate,
        ...
        public void ExtractZip(Stream inputStream, string targetDirectory,
                       ...
                       bool isStreamOwner)
        {
            if ((overwrite == Sobreescritura.Preguntar) && (confirmDelegate == null))
            {
        ...
        {
            bool proceed = true;
            if (overwrite_ != Sobreescritura.Siempre)
            {
                if (File.Exists(targetName))
                {
                    if ((overwrite_ == Sobreescritura.Preguntar) && (confirmDelegate_ != null))
                    {
            ...
            if (proceed)
            {
                ...
                if (continueRunning_)
                {
                    try
                    {
                        //Para el caso de que ya exista un fichero oculto o de solo lectura, y que se tenga que sobreescribir
                        //Si se dejan los atributos modificados, la funcion File.Create(targetName) de más abajo da error (UnauthorizedAccessException)
                        if (File.Exists(targetName))
                        {
                            FileAttributes attributes = File.GetAttributes(targetName);

                            //Si está oculto, le quitamos ese atributo
                            if ((attributes & FileAttributes.Hidden) == FileAttributes.Hidden)
                            {
                                attributes = this.RemoveAttribute(attributes, FileAttributes.Hidden);
                                File.SetAttributes(targetName, attributes);
                            }

                            //Si está como sólo lectura, le quitamos ese atributo
                            if ((attributes & FileAttributes.ReadOnly) == FileAttributes.ReadOnly)
                            {
                                attributes = this.RemoveAttribute(attributes, FileAttributes.ReadOnly);
                                File.SetAttributes(targetName, attributes);
                            }
                        }

                        using (FileStream outputStream = File.Create(targetName))//, FileOptions.Asynchronous))
                        {
                            if (buffer_ == null)
                            {
                                buffer_ = new byte[4096];
                    ...
                    catch (OCompresionFicherosExcepcionContraseñaInválida ocfeci)
                    {
                        throw ocfeci;
                    }
                    catch (OCompresionFicherosExcepcionContraseñaNoProporcionada ocfecnp)
                    {
                        throw ocfecnp;
                    }
                    catch (Exception ex)
        ...
        public void Dispose()
        {
            ...
            this.confirmDelegate_ = null;
            this.events_ = null;
            this.entryFactory_ = null;
            this.extractNameTransform_ = null;
        }
