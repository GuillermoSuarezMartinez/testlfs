...
{
    public partial class OrbitaRemotingMultidispositivo : UserControl
    {
        #region Delegados
        /// <summary>
        ...
        /// </summary>
        internal int _numeroServidor;
        #endregion

        public OrbitaRemotingMultidispositivo()
        {
            InitializeComponent();
        }
        ...
        public virtual void Iniciar()
        {
            try
            {
                ...
                this._numeroServidor = Convert.ToInt32(this.txtNumeroServidor.Text);
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                int[] puertos = new int[1] { (this._remotingPuerto) };
                string[] servidores = new string[1] { this._servidorRemoting };
        ...
        public virtual void cambiarEstado(OEstadoComms estado)
        {
            ...
            if (estado.Estado == "OK")
            {
                ...
                {
                    //this.txtNumeroServidor.BackColor = System.Drawing.Color.Green;
                }
            }
            else
            {
                if (estado.Id == idDispositivo)
                {
                    //this.txtNumeroServidor.BackColor = System.Drawing.Color.Red;
                }
            }
        }
        /// <summary>
        ...
        /// <summary>
        /// Realiza la lectura y la vuelca al PLC
        /// </summary>
        private void Leer()
        {
            try
        ...
        private void btnFiltrar_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.txtFiltro.Text != string.Empty)
                {
                    DataTable dt = (DataTable)this.dataGridViewOrbita.DataSource;
                    DataRow[] filtrado = dt.Select("Variable like  '" + txtFiltro.Text + "%'");
                    ...
                    foreach (DataRow item in filtrado)
                    {
                        DataRow dr = dtFilter.NewRow();
                        dr["Variable"] = item["Variable"];
                        try
                        {
                            dr["Valor"] = item["Valor"];
                        }
                    ...
                    this.dataGridViewOrbita.DataSource = dtFilter;
                }
                else
                {
                    this.Leer();
            ...
            {
                MessageBox.Show("No se puede realizar el filtrado");
            }
        }
        private void cmbDispositivo_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                this._idDispositivo = Convert.ToInt32(this.cmbDispositivo.Text);
            }
            catch (Exception ex)
            {
