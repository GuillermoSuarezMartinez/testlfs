...
namespace Orbita.VA.Hardware
{
    ...
    internal class ImagePylon
    {
        ...
        public ImagePylon(int newWidth, int newHeight, Byte[] newBuffer, bool color)
        {
            Width = newWidth;
            Height = newHeight;
            Buffer = newBuffer;
    ...
    /// </summary>
    internal class GrabResultPylon
    {
        #region Atributo(s)
		public ImagePylon ImageData; /* Holds the taken image. */
    ...
    internal class ImageProvider
    {
        ...
        protected List<GrabResultPylon> m_grabbedBuffers; /* List of grab results already grabbed. */
        ...
        public ImageProvider()
        {
            ...
            m_grabbedBuffers = new List<GrabResultPylon>();
            /* Create handles. */
        ...
        protected void EnqueueTakenImage(PylonGrabResult_t grabResult)
        {
            ...
            GrabResultPylon newGrabResultInternal = new GrabResultPylon();
            ...
            {
                newGrabResultInternal.ImageData = new ImagePylon(grabResult.SizeX, grabResult.SizeY, buffer.Array, grabResult.PixelType == EPylonPixelType.PixelType_RGBA8packed);
            }
            else /* Conversion is required. */
            {
                ...
                /* Add the image data. */
                newGrabResultInternal.ImageData = new ImagePylon(grabResult.SizeX, grabResult.SizeY, convertedBuffer.Array, m_converterOutputFormatIsColor);
            }
            lock (m_lockObject) /* Lock the grab result queue to avoid that two threads modify the same data. */
            {
        ...
        public void Stop()
        {
            if (m_open && m_grabThread.IsAlive) /* Only start when open and grabbing. */
            {
                ...
                bool cerrado = false;
                while (!cerrado && m_grabThread.IsAlive)
                {
                    cerrado = m_grabThread.Join(10);
        ...
        /// </summary>
        /// <returns></returns>
        public ImagePylon GetCurrentImage()
        {
            lock (m_lockObject) /* Lock the grab result queue to avoid that two threads modify the same data. */
            {
        ...
        /// An image is available when the ImageReady event is fired. */
        /// </summary>
        public ImagePylon GetLatestImage()
        {
            lock (m_lockObject) /* Lock the grab result queue to avoid that two threads modify the same data. */
            {
        ...
        /// <returns></returns>
        public long setHeartbeatTimeout(long timeout_ms)
        {
            ...
            hNodemap = Pylon.DeviceGetTLNodeMap(m_hDevice);
