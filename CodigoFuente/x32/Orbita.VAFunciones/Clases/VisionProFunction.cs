...
namespace Orbita.VAFunciones
{
    ...
    /// </summary>
    public class VisionProFunction : VisionFunctionBase
    {
        #region Constante(s)
        /// <summary>
        ...
        /// </summary>
        public VisionProFunction(string codFuncionVision)
            : base(codFuncionVision)
        {
            ...
            {
                LogsRuntime.Error(ModulosFunciones.VisionPro, this.Codigo, exception);
            }
        }
        ...
        private void FinEjecucionThread(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            {
                ThreadRuntime.SincronizarConThreadPrincipal(new RunWorkerCompletedEventHandler(this.FinEjecucionThread), new object[] { sender, e });
                return;
            }
        ...
        public override bool SetEntrada(string codigo, object valor, EnumTipoDato tipoVariable)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        if (valor is VisionProImage)
                        {
                            VisionProImage visionProImage = (VisionProImage)valor;
                            this.ToolBlock.Inputs[codigo].Value = visionProImage.Image;
                            functionReturnValue = true;
                        }
                        else
                        {
                            OrbitaImage orbitaImage = (OrbitaImage)valor;
                            VisionProImage visionProImage;
                            if (orbitaImage.Convert<VisionProImage>(out visionProImage))
                            {
                                this.ToolBlock.Inputs[codigo].Value = visionProImage.Image;
                                functionReturnValue = true;
                            }
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public override bool GetSalida(string codigo, out object valor, EnumTipoDato tipoVariable)
        {
            valor = null;
            bool functionReturnValue = false;
            try
            {
                ...
                switch (tipoVariable)
                {
                    case EnumTipoDato.Imagen:
                        {
                            VisionProImage visionProImage = new VisionProImage(objValor);
                            valor = visionProImage;
                            break;
                        }
                    case EnumTipoDato.Grafico:
                        {
                            VisionProGrafico visionProGrafico = new VisionProGrafico(objValor);
