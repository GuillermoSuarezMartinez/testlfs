...
namespace Orbita.Comunicaciones
{
    ...
    public class OProtocoloTCPSiemensGateTraffic : OProtocoloTCPSiemens
    {
        ...
        #region Propiedades
        /// <summary>
        /// Identificador del mensaje TRAFFIC data.
        /// </summary>
        public byte[] TRAData
        {
        ...
        /// <summary>
        /// Identificador del mensaje TRAFFIC data result.
        /// </summary>
        ...
        public override byte[] KeepAliveEnviar()
        {
            ...
            Array.Copy(TRAData, 0, resultado, 1, TRAData.Length);
        ...
        public override bool KeepAliveProcesar(byte[] bytesRecibidos, out byte[] bytesLecturas)
        {
            ...
            if (bytesRecibidos[0] == STX[0] && bytesRecibidos[FinTramaKeepAliveRecepcion] == CR[0] && bytesRecibidos.Length == MaximaLongitudMensaje)
            {
                ...
                Array.Copy(bytesRecibidos, 17, bytesLecturas, 0, 9);
                Array.Copy(bytesRecibidos, 27, bytesLecturas, 9, 3);
                bcc[0] = id;
        ...
        /// </summary>
        /// <param name="bytesSalidas">Salidas a procesar.</param>
        /// <param name="idMensaje">Identificador del mensaje.</param>
        /// <returns></returns>
        public override byte[] SalidasEnviar(byte[] bytesSalidas, byte idMensaje)
        {
            byte[] resultado = KeepAliveEnviar();
            resultado[9] = idMensaje;

            Array.Copy(bytesSalidas, 0, resultado, 11, 3);

            var bcc = new byte[4];

            bcc[0] = idMensaje;
            Array.Copy(bytesSalidas, 0, bcc, 1, 3);
        ...
        public override bool SalidasProcesar(byte[] bytesRecibidos, byte id, out byte[] bytesLecturas)
        {
            ...
            if (bytesRecibidos[0] == STX[0] && bytesRecibidos[FinTramaKeepAliveRecepcion] == CR[0] && bytesRecibidos.Length == MaximaLongitudMensaje)
            {
                Array.Copy(bytesRecibidos, 17, entradas, 0, 9); // Entradas.
                Array.Copy(bytesRecibidos, 27, salidas, 0, 3);  // Salidas.
                ...
                Array.Copy(entradas, 0, bytesLecturas, 0, 9); // Entradas resultado.
                Array.Copy(salidas, 0, bytesLecturas, 9, 3);  // Salidas resultado.
